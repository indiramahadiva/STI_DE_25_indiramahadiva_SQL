...
-- both rows will not be inserted
DROP TABLE students
-- UNIQUE constraint: all values in a column must be different
CREATE TABLE
    students (name VARCHAR UNIQUE, age INTEGER);
...
-- both rows will have value of 2025 in the second column
DROP TABLE students
/* ===================
Enforce constraints 
with ALTER TABLE
...
-- DEFAULT constraint
CREATE TABLE
    students (name VARCHAR, age INTEGER);
